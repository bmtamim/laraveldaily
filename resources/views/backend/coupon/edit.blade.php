...
@push('styles')
    <link rel="stylesheet" href="{{ asset('backend/lib/datetimepicker/jquery.datetimepicker.min.css') }}">
@endpush
@section('breadcrumb')
    <nav class="breadcrumb sl-breadcrumb">
        ...
        <span class="breadcrumb-item active">{{ __('Edit Coupons') }}</span>
    </nav>
@endsection
@section('content')
    <div class="row">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="card mb-4">
                            <div class="card-header">
                                <h6 class="m-0">{{ __('Title ,Coupon Code & Description ') }}</h6>
                            </div>
                            <div class="card-body">
                                <div class="form-group">
                                    <input type="text" name="name" id="name" class="form-control"
                                           placeholder="Coupon Title" value="{{ $coupon->name ?? old('name') }}">
                                </div>
                                <div class="form-group">
                                    <input type="text" name="code" id="code" class="form-control"
                                           placeholder="Coupon Code" value="{{ $coupon->code ?? old('code') }}">
                                ...
                                <textarea name="description" id="description" rows="4" class="form-control"
                                          placeholder="Coupon Description">{{ $coupon->description ?? old('description') }}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="card ">
                                ...
                                <div class="tab-content" id="nav-tabContent">
                                        ...
                                        <div class="row py-3 align-items-center">
                                            ...
                                            <div class="col-sm-9">
                                                <select name="discount_type" id="discount-type" class="form-control">
                                                    <option value="percentage" {{ isset($coupon) ? ($coupon->discount_type == 'percentage' ? 'selected' : '') : (old('discount_type') == 'percentage' ? 'selected' : '') }}>{{ __('Percentage Discount') }}</option>
                                                    <option value="fixed_cart" {{ isset($coupon) ? ($coupon->discount_type == 'fixed_cart' ? 'selected' : '') : (old('discount_type') == 'fixed_cart' ? 'selected' : '') }}>{{ __('Fixed Cart Discount') }}</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row py-3 align-items-center">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="number" name="discount_amount" id="discount_amount"
                                                       class="form-control" placeholder="0" value="{{ $coupon->discount_amount ?? old('discount_amount') }}">
                                            </div>
                                        </div>
                                        <div class="row py-3 align-items-center">
                                            <div class="col-sm-3">
                                    ...
                                    <div class="tab-pane fade" id="usage-restriction" role="tabpanel"
                                        ...
                                        <div class="row py-3 align-items-center">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="number" name="minimum_spend" id="minimum_spend"
                                                       class="form-control" value="{{ $coupon->minimum_spend ?? old('discount_amount') }}">
                                            </div>
                                        </div>
                                        <div class="row py-3 align-items-center">
                                            <div class="col-sm-3">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="number" name="maximum_spend" id="maximum_spend"
                                                       class="form-control" value="{{ $coupon->maximum_spend ?? old('discount_amount') }}">
                                            </div>
                                        </div>
                                        <div class="row py-3 align-items-center">
                                            <div class="col-sm-3">
                                            ...
                                            <div class="col-sm-9">
                                                <label class="ckbox">
                                                    <input type="checkbox" name="individual_use" id="individual_use" {{ isset($coupon) ? ($coupon->individual_use == true ? 'checked' : '') : (old('individual_use') == true ? 'checked' : '') }}>
                                        ...
                                        <div class="row py-3 align-items-center">
                                            ...
                                            <div class="col-sm-9">
                                                <label class="ckbox">
                                                    <input type="checkbox" name="exclude_sale" id="exclude_sale" {{ isset($coupon) ? ($coupon->exclude_sale == true ? 'checked' : '') : (old('exclude_sale') == true ? 'checked' : '') }}>
...
@push('scripts')
    <script src="{{ asset('backend/lib/datetimepicker/jquery.datetimepicker.full.min.js') }}"></script>
    <script>
        ...
        jQuery('#coupon_enable').datetimepicker({
            ...
            yearStart: 2020,
            step: 30,
            defaultDate : "{{ \Carbon\Carbon::parse($coupon->coupon_enable)->format('d-m-Y') }}",
            defaultTime : "{{ \Carbon\Carbon::parse($coupon->coupon_enable)->format(' h:i a') }}",
        ...
        jQuery('#coupon_expiry').datetimepicker({
            ...
            yearStart: 2020,
            step: 30,
            defaultDate : "{{ \Carbon\Carbon::parse($coupon->coupon_expiry)->format('d-m-Y') }}",
            defaultTime : "{{ \Carbon\Carbon::parse($coupon->coupon_expiry)->format(' h:i a') }}",
